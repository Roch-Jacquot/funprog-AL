...
case object ParseAndValidateService {
  ...
  /**
   * extractGardenSizeFromString returns the formated point that represents the
   * limits of the garden GardenSize is a type alias of point
   * @param rawPoint
   * @return
   *   Try[GardenSize]
   */
    ...
    Try(
      rawPositionsAndInstructions
        ...
        .map { lines =>
          ...
          val instructions = lines(INSTRUCTIONS_LINE)
            .split(EMPTY_SEPARATOR)
      ...
      .flatMap(mowers => {
        ...
        val negativeValues = debutPoints
          .flatMap(positionAndDirection =>
            List(positionAndDirection.point.x, positionAndDirection.point.y)
          )
          .filter(_ < MIN_POSITION)
