...
object YamlConverters {
  trait YamlConverter[T] {
     ...
     * @param value
     * @return
     *   YamlValue of a particular type
     */
  ...
  implicit object PositionAndDirectionConverter
      extends YamlConverter[PositionAndDirection] {
    override def convert(
        positionAndDirection: PositionAndDirection): YamlValue =
      YamlObject(
        Map(
  ...
  implicit object MowerAfterMovementConverter
      extends YamlConverter[MowerAfterMovement] {
      ...
      YamlObject(
        Map(
          ...
          "instructions" -> YamlArray(
            mowerAfterMovement.instructions.map(instruction =>
              YamlString(instruction.entryName)
            )
          ),
    ...
    override def convert(funProgResult: FunProgResult): YamlValue =
      YamlObject(
        Map(
          ...
          "tondeuses" -> YamlArray(
            funProgResult.tondeuses.map(tondeuse => tondeuse.toYaml)
          )
        )
      )
  }
  ...
  implicit class YamlOps[T](value: T) {
    ...
    /**
     * toYaml takes a value and a converter and applies the convert function on
     * it
     * @param converter
     * @return
     */
